...
input_size = 28
hidden_size1 = 512
fc_hidden_size = [128, 512, 512, 128]
num_layers = 1
num_classes = 28
batch_size = 32
num_epochs = 60
...
class RNN(nn.Module):
    def __init__(self, input_size, hidden_size1, num_layers, num_classes):
        ...
        self.lstm1 = nn.LSTM(fc_hidden_size[1], hidden_size1, num_layers, batch_first = True, dropout = 0)
        self.fc4 = nn.Linear(hidden_size1, fc_hidden_size[2])
        ...
        with torch.no_grad():
            ...
            if (i+1) % 40 == 0:
                ...
                if loss < min_l:
                    min_l = loss
                    if loss < 0.21:
                        torch.save(model.state_dict(), 'model_lstm2_v22_{:.4f}_{:.4f}.ckpt'.format(auc[1], loss))
